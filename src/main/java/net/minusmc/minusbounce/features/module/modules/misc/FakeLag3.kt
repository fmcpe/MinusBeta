...
@ModuleInfo(name = "FakeLag3", spacedName = "Fake Lag", description = "Abuse latency.", category = ModuleCategory.MISC)
class FakeLag3 : Module() {
    private var serverPosition: Vec3? = null
    private val inboundPackets = ConcurrentLinkedQueue<TimePacket>()
    private val outboundPackets = ConcurrentLinkedQueue<TimePacket>()
    ...
    fun onPacket(e: PacketEvent) {
        ...
        if (e.eventType == EventState.RECEIVE) {
            inboundPackets.add(TimePacket(e.packet))
            e.isCancelled = true
            e.stopRunEvent = true
        ...
        if (e.eventType == EventState.SEND) {
            outboundPackets.add(TimePacket(e.packet))
            e.isCancelled = true
            e.stopRunEvent = true
...
data class TimePacket(val packet: Packet<*>, val time: Long = System.currentTimeMillis())